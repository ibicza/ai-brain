"""Build the genuine M-33.6e pool from metadata-only network probes."""
...
def main() -> None:
    ...
    if head != args.q20_sha or len(head) != 40 or status:
        raise ValueError("metadata probe requires a clean exact-Q20 worktree")
    ...
    _write(
        args.output / "denylist_snapshot.json",
        build_m336e_disclosed_identity_denylist(),
    )
