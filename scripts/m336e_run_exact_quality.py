"""Run exact-commit M-33.6e quality gates and preserve canonical evidence."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--phase", choices=("R20", "Q20", "F20", "H20", "E20"), required=True
    )
    ...
    if args.output.exists():
        raise FileExistsError("fresh exact M-33.6e quality output already exists")
    head = _run(("git", "rev-parse", "HEAD^{commit}"), root)[1].strip()
    if head != args.expected_head or len(head) != 40:
        raise ValueError("exact M-33.6e quality HEAD mismatch")
    ...
    if classes.exists():
        ...
        classes.rmdir()
    if not passed:
        raise SystemExit("exact M-33.6e quality gate failed")
