"""Run exact M-33.6k quality gates and emit a public-safe receipt."""
...
def main() -> None:
    ...
    if output.exists() or logs.exists():
        raise FileExistsError("M336K quality destination must be fresh")
    if output.is_relative_to(repository) or logs.is_relative_to(repository):
        raise ValueError("M336K quality evidence must remain outside Git")
    ...
    if head != args.expected_sha or status:
        raise ValueError("M336K quality requires a clean exact commit")
    ...
    checks = [
        ...
        _check(
            ...
            (
                ...
                "tests/test_m336i_authorized_final_java_route.py",
                "-k",
                "network or acquisition or provider or candidate",
            ),
            repository,
            logs,
        ),
        _check(
            ...
            (
                ...
                "-c",
                (
                    "import sys; import ai_brain.stage3.acquisition.m336k_acquisition; "
    ...
    with tempfile.TemporaryDirectory(prefix="m336k-quality-") as temporary:
    ...
    body = {
        "schema_version": 1,
        "contract_role": "PUBLIC_SAFE_M336K_QUALITY_RECEIPT",
    ...
    if receipt["status"] != "PASS":
        raise ValueError("M336K exact quality failed")
