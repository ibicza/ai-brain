"""Execute the exact public-safe M-33.6k.5 identity mutation matrix."""
...
from ai_brain.stage3.acquisition.m336k5_request import (
    ...
    load_m336k5_final_route_request,
    write_m336k5_final_route_request,
)
from ai_brain.stage3.acquisition.m336k5_request import (
    validate_m336k5_final_invocation as _validate_m336k5_final_invocation,
)
from ai_brain.stage3.acquisition.m336k5_startup import startup_receipt_from_path

_STARTUP_RECEIPT_PATH: Path | None = None


def validate_m336k5_final_invocation(path: Path):
    if _STARTUP_RECEIPT_PATH is None:
        raise RuntimeError("M336K5 mutation startup receipt is not configured")
    return _validate_m336k5_final_invocation(
        path, startup_receipt_path=_STARTUP_RECEIPT_PATH
    )
...
def main() -> None:
    ...
    global _STARTUP_RECEIPT_PATH
    _STARTUP_RECEIPT_PATH = args.startup_receipt.resolve(strict=True)
    startup_receipt_from_path(_STARTUP_RECEIPT_PATH)
    ...
    if workspace.exists() or output.exists() or output.is_relative_to(workspace):
        raise M336K2ProtocolError("M336K5 mutation destinations are not fresh")
...
def run_mutation_matrix(request_path: Path, workspace: Path) -> dict:
    ...
    bundle = M336K5RouteIdentityBundle.from_dict(bundle_value)
    cases: list[dict] = []
    ...
    rejected(
        ...
        lambda: validate_m336k5_final_invocation(
            mutated_request(
                "01-historical-f28",
    ...
    cross_types = (
        (
            ...
            "protocol_run_id",
            M336K5RouteVersion(
                "m336k5.candidate-isolated-java-final-route.v1"
            ).canonical_object(),
        ),
        (
            3,
            ...
            "route_version",
            M336K5ProtocolRunId("m336k5.final-java.outcome-a.v1").canonical_object(),
        ),
        (
            4,
            ...
            "protocol_run_id",
            M336K5AcquisitionRunId(
                "m336k5.final-java.global-acquisition.v1"
            ).canonical_object(),
        ),
        (
            5,
            ...
            "evaluator_run_id",
            M336K5SelectorRunId("m336k5.final-java.selector.v1").canonical_object(),
        ),
        (
            6,
            "evaluator_id_in_selector_field",
            "selector_run_id",
            M336K5EvaluatorRunId("m336k5.final-java.evaluator.v1").canonical_object(),
        ),
    )
    for case_id, name, field, identity in cross_types:
        rejected(
            ...
            lambda name=name, field=field, identity=identity: (
                validate_m336k5_final_invocation(mutated_bundle(name, field, identity))
            ),
            "type was confused",
        )
    rejected(
    ...
    for case_id, name, value in protocol_mutations:
        rejected(
            ...
            name,
            "TYPED_BUNDLE_LOADER",
            lambda name=name, value=value: validate_m336k5_final_invocation(
                mutated_bundle(
                    name,
                    "protocol_run_id",
    ...
    changed_authorization["protocol_run_id_typed"] = M336K5ProtocolRunId(
        "m336k5.disposable.authorization-mismatch.v1"
    ).canonical_object()
    ...
    rejected(
        ...
        lambda: M336K5IdentityCheckingWorker(
    ...
    rejected(
        ...
        lambda: verify_m336k5_route_ledger_identity(
            ledger,
            bundle=bundle,
    ...
    rebuilt = build_m336k5_final_route_request(**builder_values)
    rebuilt_path = workspace / "canonical-rebuilt-request.json"
    write_m336k5_final_route_request(rebuilt, rebuilt_path)
    if rebuilt != request:
        raise M336K2ProtocolError("M336K5 canonical request builder is not stable")

    validated = validate_m336k5_final_invocation(rebuilt_path)
    ...
    if any(path.exists() for path in destination_paths):
        raise M336K2ProtocolError("M336K5 mutation preflight created a destination")
    ...
    body = {
        "schema_version": 1,
        "contract_role": "PUBLIC_SAFE_M336K5_IDENTITY_MUTATION_MATRIX",
    ...
    if receipt["status"] != "PASS":
        raise M336K2ProtocolError("M336K5 identity mutation matrix failed")
    return receipt
...
def _object(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise M336K2ProtocolError("M336K5 mutation input is not an object")
    return value
