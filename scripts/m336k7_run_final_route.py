"""Validate or execute the canonical M-33.6k.7 final route."""
...
from ai_brain.stage3.acquisition.m336k7_request import (
    build_m336k7_internal_stage_request,
    load_m336k7_preledger_receipt,
    validate_m336k7_final_invocation,
...
def main() -> None:
    ...
    if args.validate_only:
        if (
            ...
            raise M336K2ProtocolError(
                "M336K7 validate-only release inputs are forbidden"
            )
        validated = validate_m336k7_final_invocation(
        ...
        if (
            ...
            raise M336K2ProtocolError("M336K7 execution release inputs are absent")
        release_path = args.reservation_release_receipt.resolve(strict=False)
        if release_path.exists():
            raise M336K2ProtocolError("M336K7 reservation release output is stale")
        release_path.parent.mkdir(parents=True, exist_ok=True)
        _claim_execution_lock(release_path, request_path)
        prior = load_m336k7_preledger_receipt(
        ...
        if validated.receipt != prior:
            raise M336K2ProtocolError(
                "M336K7 execution differs from post-freeze validation"
            )
        if validated.request.purpose == "QUALIFICATION":
            raise M336K2ProtocolError("M336K7 qualification request is not executable")
        reservation_file = Path(validated.request.storage_reservation_file).resolve(
            strict=True
        )
        free_before_release = shutil.disk_usage(reservation_file.parent).free
        release_m336k5_storage_reservation(reservation_file)
        free_after_release = shutil.disk_usage(reservation_file.parent).free
    ...
    if args.validation_receipt is not None:
        ...
        if output.exists():
            raise FileExistsError("M336K7 validation receipt output must be fresh")
    ...
    internal_request = build_m336k7_internal_stage_request(validated)
    private_root = Path(request.private_root).resolve(strict=False)
    private_root.mkdir(parents=True)
    monitor = M336K5ResourceMonitor(
        ledger=private_root / "m336k7-resource-samples.jsonl",
    ...
    verify_m336k5_route_ledger_identity(
        ledger,
        bundle=validated.bundle,
        exact_f30_sha=request.exact_f32_sha,
...
def _component_object(root: Path, components: dict, name: str) -> dict:
    ...
    if type(value) is not dict:
        raise TypeError(f"M336K7 frozen component is not an object: {name}")
    return value
...
def _claim_execution_lock(release_path: Path, request_path: Path) -> None:
    ...
    try:
        ...
        raise M336K2ProtocolError(
            "M336K7 exclusive execution lock already exists"
        ) from error
