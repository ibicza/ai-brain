...
def main() -> None:
    ...
    result = prepare_m336k4_karina_capsule(_object(args.request.resolve(strict=True)))
    print(canonical_json(result))


def prepare_m336k4_karina_capsule(request: dict) -> dict:
    expected = {
        "repository",
        "exact_head",
        "git_executable",
    ...
    if output.exists() or output.is_relative_to(repository):
        raise M336K2ProtocolError("M336K4 Karina preparation output is not fresh")
    ...
    if head != exact_head or status:
        raise M336K2ProtocolError("M336K4 Karina source is not clean exact HEAD")
    ...
    if set(base) != required_capsule_fields:
        raise M336K2ProtocolError("M336K4 base Karina capsule fields changed")
    capsule = {
        **base,
    ...
    try:
        ...
        if first != second:
            raise M336K2ProtocolError("M336K4 Karina capsule metadata is unstable")
        ...
        body = {
            "schema_version": 1,
            "contract_role": "PUBLIC_SAFE_M336K4_KARINA_PREPARATION_RECEIPT",
...
def _remote_metadata(
    *,
    ...
    environment = (
        "LC_ALL=C.UTF-8 PATH=/usr/bin:/bin PIP_NO_INDEX=1 "
        "PYTHONDONTWRITEBYTECODE=1 PYTHONNOUSERSITE=1 PYTHONPATH="
        + shlex.quote((remote_repository / "src").as_posix())
        + " TZ=UTC UV_OFFLINE=1"
    )
    command = (
        environment
        + " exec "
        + shlex.quote(capsule["python_executable"])
        + " -B "
        + shlex.quote(
            (
                remote_repository / "scripts/m336k4_export_karina_capsule_metadata.py"
            ).as_posix()
        )
        + " --private-capsule "
        + shlex.quote(remote_capsule.as_posix())
    )
    raw = _ssh(ssh, ssh_options, endpoint, command)
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as error:
        raise M336K2ProtocolError("M336K4 Karina metadata is not JSON") from error
    expected = {
        "schema_version",
        "contract_role",
    ...
    if (
        ...
        or value.get("contract_role") != "PUBLIC_SAFE_M336K4_KARINA_CAPSULE_METADATA"
    ):
        raise M336K2ProtocolError("M336K4 Karina metadata failed verification")
    return value


def _remote_workspace(value: object) -> PurePosixPath:
    if not isinstance(value, str):
        raise M336K2ProtocolError("M336K4 Karina workspace is invalid")
    path = PurePosixPath(value)
    if (
        not path.is_absolute()
        or len(path.parts) != 4
        or path.parts[:3] != ("/", "home", "ibicza")
        or not path.name.startswith("m336k4-")
        or re.fullmatch(r"[a-z0-9-]+", path.name) is None
    ):
        raise M336K2ProtocolError("M336K4 Karina workspace is unsafe")
    return path
...
def _object(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise M336K2ProtocolError("M336K4 Karina JSON input is not an object")
    return value
