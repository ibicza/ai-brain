"""Stage the evidence-only E26 subset through its V2 contract."""
...
def main() -> None:
    ...
    if not isinstance(request, dict) or set(request) != _FIELDS:
        raise ValueError("M336J E26 publisher request fields changed")
    output = Path(request["output"])
    result = stage_m336j_e26_publication(
        repository=Path(request["repository"]),
        git_executable=Path(request["git_executable"]),
        exact_f26_sha=request["exact_f26_sha"],
        exact_h26_sha=request["exact_h26_sha"],
        freeze_manifest=Path(request["freeze_manifest"]),
        evidence_source=Path(request["evidence_source"]),
        output=output,
    )
    write_canonical_json(output / "e26_publication_report.json", result)
