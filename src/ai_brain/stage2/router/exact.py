...
from ai_brain.stage2.facts.models import FactQuery
...
class ExactUnifiedRouter:
    ...
    def dependencies(self) -> DependencySnapshot:
        ...
        return DependencySnapshot(
            ...
            skill_registry_hash=(registry.manifest.registry_hash if registry else None),
            rule_memory_hash=(registry.manifest.rule_memory_hash if registry else None),
    ...
    def _controlled(
        ...
        if len(complete) == 1:
            ...
            return make_decision(
                ...
                parser_evidence={**item.evidence, "payload": item.payload},
                ambiguity_fields=(),
                next_action=action,
                dependencies=dependencies,
            )
