...
from ai_brain.stage3.acquisition.m336k4_registry import build_m336k4_route_registry

M336K4_FINAL_REQUEST_CONTRACT = {
    "schema_version": 2,
    "contract_role": "M336K4_CANONICAL_FINAL_ROUTE_REQUEST_V2",
...
M336K4_FINAL_REQUEST_BUILDER_HASH = content_hash(M336K4_FINAL_REQUEST_CONTRACT)
_SHA_OR_ZERO = re.compile(r"(?:[0-9a-f]{40})")
_HASH = re.compile(r"[0-9a-f]{64}")


@dataclass(frozen=True)
class M336K4FinalRouteRequestV2:
    schema_version: int
    contract_role: str
    purpose: str
    repository: str
    ...
    def from_dict(cls, value: dict) -> Self:
        if type(value) is not dict or set(value) != set(cls.__dataclass_fields__):
            forbidden = (
                set(value)
                & set(M336K4_FINAL_REQUEST_CONTRACT["forbidden_external_fields"])
                if isinstance(value, dict)
                else set()
            )
            if forbidden:
                raise M336K2ProtocolError(
                    "M336K4 external request contains a caller-selected identity"
                )
            raise M336K2ProtocolError("M336K4 final request fields changed")
        result = cls(**value)
        result.verify()
        return result
    ...
    def verify(self) -> None:
        if (
            ...
            or self.contract_role != "M336K4_CANONICAL_FINAL_ROUTE_REQUEST_V2"
            or self.purpose not in {"DISPOSABLE", "OFFICIAL", "QUALIFICATION"}
            or _SHA_OR_ZERO.fullmatch(self.exact_implementation_sha) is None
            or _SHA_OR_ZERO.fullmatch(self.exact_f29_sha) is None
            or self.builder_identity_hash != M336K4_FINAL_REQUEST_BUILDER_HASH
            or _HASH.fullmatch(self.request_hash) is None
            or content_hash(self._body()) != self.request_hash
        ):
            raise M336K2ProtocolError("M336K4 canonical final request is invalid")


@dataclass(frozen=True)
class M336K4PreLedgerInvocationReceipt:
    schema_version: int
    contract_role: str
    exact_implementation_sha: str
    prospective_or_exact_f29_sha: str
...
@dataclass(frozen=True)
class M336K4ValidatedInvocation:
    request: M336K4FinalRouteRequestV2
    bundle: M336K4RouteIdentityBundle
    authorization: M336K4FinalAuthorization
    freeze: M336K4FreezeManifest
    receipt: M336K4PreLedgerInvocationReceipt


def build_m336k4_final_route_request(**values) -> M336K4FinalRouteRequestV2:
    """Only tracked constructor for disposable, qualification, and official JSON."""

    forbidden = set(values) & set(
        M336K4_FINAL_REQUEST_CONTRACT["forbidden_external_fields"]
    )
    if forbidden:
        raise M336K2ProtocolError(
            "M336K4 request builder rejects caller-selected route identities"
        )
    body = {
        "schema_version": 2,
        "contract_role": "M336K4_CANONICAL_FINAL_ROUTE_REQUEST_V2",
        **values,
        "builder_identity_hash": M336K4_FINAL_REQUEST_BUILDER_HASH,
    }
    if set(body) != set(M336K4FinalRouteRequestV2.__dataclass_fields__) - {
        "request_hash"
    }:
        raise M336K2ProtocolError("M336K4 request builder arguments changed")
    result = M336K4FinalRouteRequestV2(**body, request_hash=content_hash(body))
    result.verify()
    return result


def write_m336k4_final_route_request(
    request: M336K4FinalRouteRequestV2, output: Path
) -> None:
    request.verify()
    if output.exists():
        raise FileExistsError("M336K4 canonical request output must be fresh")
...
def load_m336k4_final_route_request(path: Path) -> M336K4FinalRouteRequestV2:
    raw = path.resolve(strict=True).read_bytes()
    value = _strict_object(raw)
    request = M336K4FinalRouteRequestV2.from_dict(value)
    if raw != (canonical_json(value) + "\n").encode("utf-8"):
        raise M336K2ProtocolError("M336K4 request serialization is not canonical")
    return request


def validate_m336k4_final_invocation(
    request_path: Path,
) -> M336K4ValidatedInvocation:
    ...
    authorization = M336K4FinalAuthorization.from_dict(_object(authorization_path))
    ...
    freeze = M336K4FreezeManifest.from_dict(_object(freeze_path))
    verify_complete_m336k4_freeze(root, freeze, allow_prospective_f29=True)
    ...
    if (
        ...
        or authorization.exact_q29_sha != freeze.exact_q29_sha
        or (
            request.purpose == "DISPOSABLE"
            and bundle.protocol_run_id.value == "m336k4.final-java.outcome-a.v1"
        )
        or (
        ...
        raise M336K2ProtocolError(
            "M336K4 request/freeze/authorization cross-binding changed"
        )
    registry = build_m336k4_route_registry(root)
    if registry.registry_hash != bundle.route_registry_hash:
        raise M336K2ProtocolError(
            "M336K4 frozen route registry differs from executable source"
        )
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 route registry/manifest verification failed")
    _verify_identity_policy_components(root, components, bundle)
    _verify_executables_and_environment(root, request, components)
    ...
    body = {
        "schema_version": 2,
        "contract_role": "M336K4_SIDE_EFFECT_FREE_PRELEDGER_INVOCATION_RECEIPT",
        "exact_implementation_sha": request.exact_implementation_sha,
        "prospective_or_exact_f29_sha": request.exact_f29_sha,
    ...
    receipt = M336K4PreLedgerInvocationReceipt(**body, receipt_hash=content_hash(body))
    return M336K4ValidatedInvocation(request, bundle, authorization, freeze, receipt)


def build_m336k4_internal_stage_request(
    validated: M336K4ValidatedInvocation,
) -> dict:
    ...
    return {
        ...
        "f28_attestation": request.f29_attestation,
...
def _verify_identity_policy_components(root: Path, components: dict, bundle) -> None:
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 one-shot policy identity binding changed")


def _verify_executables_and_environment(root: Path, request, components: dict) -> None:
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 executable aliases changed")
    ...
    storage = _verified_component_object(
        root, components, "storage_budget", "receipt_hash"
    )
    if storage.get("status") != "PASS":
        raise M336K2ProtocolError("M336K4 frozen storage budget did not pass")
    parent = _nearest_existing_parent(Path(request.private_root))
    required = int(storage.get("minimum_free_bytes_windows", 0))
    if required <= 0 or shutil.disk_usage(parent).free < required:
        raise M336K2ProtocolError("M336K4 Windows storage preflight failed")
...
def _verify_destination_set(root: Path, git: Path, request) -> None:
    ...
    if set(request.final_destinations) != expected:
        raise M336K2ProtocolError("M336K4 final destination fields changed")
    raw = subprocess.run(
    ...
    if (
        ...
        raise M336K2ProtocolError(
            "M336K4 final destinations are stale, aliased, or inside Git"
        )


def _verify_attestation_and_lineage(root, git, request, freeze, bundle) -> None:
    if request.exact_f29_sha == "0" * 40:
        if request.f29_attestation is not None or request.purpose == "OFFICIAL":
            raise M336K2ProtocolError("M336K4 official request lacks exact F29")
        return
    if request.f29_attestation is None:
        raise M336K2ProtocolError("M336K4 exact F29 attestation is absent")
    value = _object(Path(request.f29_attestation).resolve(strict=True))
    if set(value) != set(M336K4CommittedFreezeAttestation.__dataclass_fields__):
        raise M336K2ProtocolError("M336K4 F29 attestation fields changed")
    attestation = M336K4CommittedFreezeAttestation(**value)
    body = dict(value)
    ...
    if (
        ...
        or attestation.exact_f29_sha != request.exact_f29_sha
        or attestation.exact_q29_parent != freeze.exact_q29_sha
        ...
        or head != upstream
        or head != remote
        or head != request.exact_f29_sha
    ):
        raise M336K2ProtocolError("M336K4 exact F29 attestation is invalid")
    if branch != request_branch_ref(request) or _git(
        git, root, "status", "--porcelain=v1"
    ):
        raise M336K2ProtocolError("M336K4 exact final lineage is not clean/pushed")
...
def _verify_karina_host_and_storage(*, request, bundle, freeze, root, components):
    ...
    if set(karina) != expected_karina_fields:
        raise M336K2ProtocolError("M336K4 Karina invocation fields changed")
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 Karina capsule/freeze binding changed")
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 Karina live preledger gate failed")
    return host, storage
...
def request_branch_ref(request: M336K4FinalRouteRequestV2) -> str:
...
def _verify_component_handle(
    ...
    if (
        ...
        raise M336K2ProtocolError(f"M336K4 frozen component handle changed: {name}")
...
def _verified_component_object(
    ...
    if component is None:
        raise M336K2ProtocolError(f"M336K4 frozen component is missing: {name}")
    ...
    if not isinstance(claimed, str) or content_hash(body) != claimed:
        raise M336K2ProtocolError(f"M336K4 component semantic hash changed: {name}")
    return value
...
def _nearest_existing_parent(path: Path) -> Path:
    ...
    while not value.exists():
        if value.parent == value:
            raise M336K2ProtocolError("M336K4 destination has no existing ancestor")
        value = value.parent
    return value
...
def _strict_object(raw: bytes) -> dict:
    def pairs(values):
        ...
        for key, value in values:
            if key in result:
                raise M336K2ProtocolError(
                    "M336K4 request contains a duplicate JSON key"
                )
            result[key] = value
        return result
    ...
    try:
        ...
        raise M336K2ProtocolError("M336K4 final request JSON is invalid") from error
    if not isinstance(value, dict):
        raise M336K2ProtocolError("M336K4 final request is not an object")
    return value
