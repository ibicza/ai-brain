"""Typed M-33.6k.4 controller with no caller-selected final identity."""
...
def run_m336k4_final_controller(
    *,
    ...
    body = {
        "schema_version": 2,
        "contract_role": "PUBLIC_SAFE_M336K4_FINAL_ROUTE_RECEIPT",
    ...
    result = M336K4FinalRouteReceipt(**body, receipt_hash=content_hash(body))
...
class M336K4IdentityCheckingWorker:
    ...
    def __call__(self, request: M336K2StageRequest) -> M336K2StageReceipt:
        if request.route_run_id != self._bundle.protocol_run_id.value:
            raise M336K2ProtocolError("M336K4 stage request changed final identity")
        ...
        try:
            ...
            raise M336K2ProtocolError(
                "M336K4 native stage response is invalid"
            ) from error
        if (
            ...
            raise M336K2ProtocolError("M336K4 worker response changed final identity")
        return result


def verify_m336k4_route_ledger_identity(
    ledger: M336K2RouteLedger,
    *,
    bundle: M336K4RouteIdentityBundle,
    exact_f29_sha: str,
    ...
    if not events or any(event.context_hash != context for event in events):
        raise M336K2ProtocolError("M336K4 route ledger context changed final identity")
    return context


def verify_m336k4_acquisition_ledger_identity(
    ...
    *,
    ...
    if not events or any(event.context_hash != context for event in events):
        raise M336K2ProtocolError(
            "M336K4 acquisition ledger context changed final identity"
        )
    return context


def verify_m336k4_selector_ledger_identity(
    ...
    *,
    ...
    if not events or any(event["context_hash"] != context for event in events):
        raise M336K2ProtocolError(
            "M336K4 selector ledger context changed final identity"
        )
    return context


def verify_m336k4_evaluator_ledger_identity(
    path: Path,
    *,
    bundle: M336K4RouteIdentityBundle,
    exact_h29_sha: str,
    ...
    if not events or events[0]["operation_hash"] != expected:
        raise M336K2ProtocolError(
            "M336K4 evaluator ledger context changed final identity"
        )
    return expected
...
def _evaluator_events(path: Path) -> tuple[dict, ...]:
    ...
    if b"\r" in raw or not raw.endswith(b"\n"):
        raise M336K2ProtocolError("M336K4 evaluator ledger is not canonical")
    result = []
    previous = None
    for ordinal, line in enumerate(raw.splitlines(), start=1):
        try:
            ...
            raise M336K2ProtocolError("M336K4 evaluator ledger is invalid") from error
        ...
        if (
            ...
            raise M336K2ProtocolError("M336K4 evaluator ledger chain changed")
        result.append(value)
        previous = claimed
    return tuple(result)


def _verify_final_identity(purpose: str, bundle: M336K4RouteIdentityBundle) -> None:
    bundle.verify()
    ...
    if bundle.execution_mode.value != "FINAL":
        raise M336K2ProtocolError("M336K4 controller execution mode is not FINAL")
    if purpose == "OFFICIAL" and bundle.protocol_run_id.value != M336K4_PROTOCOL_RUN_ID:
        raise M336K2ProtocolError("M336K4 official protocol run ID is not canonical")
...
def _verify_worker_receipt(
    ...
    if (
        ...
        raise M336K2ProtocolError("M336K4 typed worker receipt is invalid")
