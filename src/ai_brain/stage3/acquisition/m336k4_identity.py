"""Typed, non-interchangeable identities for the M-33.6k.4 final route."""
...
M336K4_ROUTE_VERSION = "m336k4.candidate-isolated-java-final-route.v1"
M336K4_PROTOCOL_RUN_ID = "m336k4.final-java.outcome-a.v1"
M336K4_ACQUISITION_RUN_ID = "m336k4.final-java.global-acquisition.v1"
M336K4_SELECTOR_RUN_ID = "m336k4.final-java.selector.v1"
M336K4_EVALUATOR_RUN_ID = "m336k4.final-java.evaluator.v1"

_HASH = re.compile(r"[0-9a-f]{64}")


@dataclass(frozen=True, eq=False)
class _M336K4Identity:
    ...
    def __post_init__(self) -> None:
        if type(self.schema_version) is not int or self.schema_version != 1:
            raise M336K2ProtocolError("M336K4 identity schema version is invalid")
        if type(self.value) is not str or not self.value:
            raise M336K2ProtocolError("M336K4 identity is empty")
        if self.value != self.value.strip():
            raise M336K2ProtocolError("M336K4 identity contains surrounding whitespace")
        if (
            not self.value.isascii()
            or unicodedata.normalize("NFKC", self.value) != self.value
        ):
            raise M336K2ProtocolError("M336K4 identity contains a Unicode confusable")
        if self.namespace.fullmatch(self.value) is None:
            raise M336K2ProtocolError(
                f"M336K4 {self.identity_kind} namespace is invalid"
            )
        if ".final-java." in self.value and self.official_value != self.value:
            raise M336K2ProtocolError(
                f"M336K4 {self.identity_kind} final identity is not canonical"
            )
    ...
    def from_dict(cls, value: dict) -> Self:
        if type(value) is not dict or set(value) != {
            ...
            raise M336K2ProtocolError(
                f"M336K4 {cls.identity_kind} codec fields changed"
            )
        if value.get("identity_kind") != cls.identity_kind:
            raise M336K2ProtocolError(f"M336K4 {cls.identity_kind} type was confused")
        result = cls(
            value=value.get("value"), schema_version=value.get("schema_version")
        )
        if value.get("identity_hash") != result.identity_hash:
            raise M336K2ProtocolError(f"M336K4 {cls.identity_kind} hash changed")
        return result


@dataclass(frozen=True, eq=False)
class M336K4RouteVersion(_M336K4Identity):
    identity_kind: ClassVar[str] = "ROUTE_VERSION"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.candidate-isolated-java-(?:final|disposable)-route\.v[1-9][0-9]*"
    )
    official_value: ClassVar[str] = M336K4_ROUTE_VERSION


@dataclass(frozen=True, eq=False)
class M336K4ProtocolRunId(_M336K4Identity):
    identity_kind: ClassVar[str] = "PROTOCOL_RUN_ID"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.(?:final-java\.outcome-a|disposable\.[a-z0-9-]+)\.v[1-9][0-9]*"
    )
    official_value: ClassVar[str] = M336K4_PROTOCOL_RUN_ID


@dataclass(frozen=True, eq=False)
class M336K4AcquisitionRunId(_M336K4Identity):
    identity_kind: ClassVar[str] = "ACQUISITION_RUN_ID"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.(?:final-java\.global-acquisition|disposable\.[a-z0-9-]+\.acquisition)\.v[1-9][0-9]*"
    )
    official_value: ClassVar[str] = M336K4_ACQUISITION_RUN_ID


@dataclass(frozen=True, eq=False)
class M336K4SelectorRunId(_M336K4Identity):
    identity_kind: ClassVar[str] = "SELECTOR_RUN_ID"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.(?:final-java\.selector|disposable\.[a-z0-9-]+\.selector)\.v[1-9][0-9]*"
    )
    official_value: ClassVar[str] = M336K4_SELECTOR_RUN_ID


@dataclass(frozen=True, eq=False)
class M336K4EvaluatorRunId(_M336K4Identity):
    identity_kind: ClassVar[str] = "EVALUATOR_RUN_ID"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.(?:final-java\.evaluator|disposable\.[a-z0-9-]+\.evaluator)\.v[1-9][0-9]*"
    )
    official_value: ClassVar[str] = M336K4_EVALUATOR_RUN_ID


@dataclass(frozen=True, eq=False)
class M336K4RouteComponentId(_M336K4Identity):
    identity_kind: ClassVar[str] = "ROUTE_COMPONENT_ID"
    namespace: ClassVar[re.Pattern[str]] = re.compile(
        r"m336k4\.route-component\.[a-z][a-z0-9-]*\.v[1-9][0-9]*"
    )


@dataclass(frozen=True, eq=False)
class M336K4ExecutionMode(_M336K4Identity):
...
class M336K4RouteIdentityBundle:
    schema_version: int
    route_version: M336K4RouteVersion
    protocol_run_id: M336K4ProtocolRunId
    acquisition_run_id: M336K4AcquisitionRunId
    selector_run_id: M336K4SelectorRunId
    evaluator_run_id: M336K4EvaluatorRunId
    execution_mode: M336K4ExecutionMode
    ...
    def from_dict(cls, value: dict) -> Self:
        if type(value) is not dict or set(value) != set(cls.__dataclass_fields__):
            raise M336K2ProtocolError("M336K4 route identity bundle fields changed")
        result = cls(
            schema_version=value.get("schema_version"),
            route_version=M336K4RouteVersion.from_dict(value.get("route_version")),
            protocol_run_id=M336K4ProtocolRunId.from_dict(value.get("protocol_run_id")),
            acquisition_run_id=M336K4AcquisitionRunId.from_dict(
                value.get("acquisition_run_id")
            ),
            selector_run_id=M336K4SelectorRunId.from_dict(value.get("selector_run_id")),
            evaluator_run_id=M336K4EvaluatorRunId.from_dict(
                value.get("evaluator_run_id")
            ),
            execution_mode=M336K4ExecutionMode.from_dict(value.get("execution_mode")),
    ...
    def verify(self) -> None:
        ...
        if (
            ...
            raise M336K2ProtocolError("M336K4 route identity bundle is invalid")


def build_m336k4_official_identity_bundle(**hashes: str) -> M336K4RouteIdentityBundle:
    return M336K4RouteIdentityBundle.build(
        route_version=M336K4RouteVersion(M336K4_ROUTE_VERSION),
        protocol_run_id=M336K4ProtocolRunId(M336K4_PROTOCOL_RUN_ID),
        acquisition_run_id=M336K4AcquisitionRunId(M336K4_ACQUISITION_RUN_ID),
        selector_run_id=M336K4SelectorRunId(M336K4_SELECTOR_RUN_ID),
        evaluator_run_id=M336K4EvaluatorRunId(M336K4_EVALUATOR_RUN_ID),
