"""Typed final authorization bound to the M-33.6k.4 route identity bundle."""
...
class M336K4FinalAuthorization:
    schema_version: int
    contract_role: str
    exact_implementation_tip: str
    exact_q29_sha: str
    branch_ref: str
    route_version_typed: M336K4RouteVersion
    protocol_run_id_typed: M336K4ProtocolRunId
    acquisition_run_id_typed: M336K4AcquisitionRunId
    selector_run_id_typed: M336K4SelectorRunId
    evaluator_run_id_typed: M336K4EvaluatorRunId
    execution_mode_typed: M336K4ExecutionMode
    ...
    def verify(self, bundle: M336K4RouteIdentityBundle | None = None) -> None:
        hashes = tuple(
            value for name, value in self._body().items() if name.endswith("_hash")
        ) + (self.authorization_hash,)
        official = self.protocol_run_id_typed.value == "m336k4.final-java.outcome-a.v1"
        branch_valid = (
            self.branch_ref == "refs/heads/exp/stage3-m336k4-typed-final-identity-v13"
            if official
            else self.branch_ref.startswith("refs/heads/disposable/m336k4-")
        )
        if (
            self.schema_version != 2
            or self.contract_role != "M336K4_TYPED_FINAL_AUTHORIZATION_V2"
            or _SHA.fullmatch(self.exact_implementation_tip) is None
            or _SHA.fullmatch(self.exact_q29_sha) is None
            or not branch_valid
            ...
            raise M336K2ProtocolError("M336K4 typed final authorization is invalid")
        if bundle is not None and (
            ...
            raise M336K2ProtocolError("M336K4 authorization/identity bundle mismatch")

    @classmethod
    def from_dict(cls, value: dict) -> Self:
        expected = set(cls.__dataclass_fields__)
        if type(value) is not dict or set(value) != expected:
            raise M336K2ProtocolError("M336K4 authorization fields changed")
        result = cls(
            **{
                **value,
                "route_version_typed": M336K4RouteVersion.from_dict(
                    value.get("route_version_typed")
                ),
                "protocol_run_id_typed": M336K4ProtocolRunId.from_dict(
                    value.get("protocol_run_id_typed")
                ),
                "acquisition_run_id_typed": M336K4AcquisitionRunId.from_dict(
                    value.get("acquisition_run_id_typed")
                ),
                "selector_run_id_typed": M336K4SelectorRunId.from_dict(
                    value.get("selector_run_id_typed")
                ),
                "evaluator_run_id_typed": M336K4EvaluatorRunId.from_dict(
                    value.get("evaluator_run_id_typed")
                ),
                "execution_mode_typed": M336K4ExecutionMode.from_dict(
...
def build_m336k4_final_authorization(
    *, bundle: M336K4RouteIdentityBundle, **values
) -> M336K4FinalAuthorization:
    body = {
        "schema_version": 2,
        "contract_role": "M336K4_TYPED_FINAL_AUTHORIZATION_V2",
    ...
    temporary = M336K4FinalAuthorization(**body, authorization_hash="0" * 64)
    result = M336K4FinalAuthorization(
