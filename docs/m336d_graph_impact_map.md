...
The final pre-commit R19 rebuild parsed 980 files and emitted 14,853 nodes / 93,594 edges; the persistent status view contains 979 files, 13,506 nodes and 91,754 edges. The depth-two E18 impact remains limited to the tracked-file view because the graph change detector intentionally excludes untracked additions until R19 exists.

The exact F19 rebuild parsed 1,009 files and emitted 15,158 nodes / 98,057 edges; the persistent status view contains 13,811 nodes and 96,147 edges. F19 only adds the frozen authority statement and metadata/policy artifacts. H19 and E19 are evidence-only additions, so they do not change implementation entry points or the pre-freeze blast-radius decision.
